...
{
	string name = T2AString(maxNode->GetName());
	return createNode(maxNode, name);
}
...
bool Exporter::findNode(const string& name, NiNodeRef& node)
{
	USES_CONVERSION;
	tstring tname = A2TString(name);
	NodeMap::iterator itr = mNameMap.find(tname);
	if (itr != mNameMap.end()) {
		node = (*itr).second;
		return true;
	}
...
bool Exporter::removeUnreferencedBones(NiNodeRef node)
{
	...
	bool remove = (nullptr != parent) && !node->IsSkinInfluence();
...
Exporter::Result Exporter::exportLight(NiNodeRef parent, INode *node, GenLight* light)
{
	...
	niLight->SetName(T2AString(node->GetName()));
...
bool Exporter::exportPrn(NiNodeRef &obj, INode *node) {
	...
	if (mSupportPrnStrings && Exporter::mNifVersionInt >= VER_10_0_1_0) {
		if (INode *parentNode = node->GetParentNode()) {
			string parentName = T2AString(parentNode->GetName());
...
{
	if (wildmatch(TEXT("Bip??"), node->GetName())) {
		return true;
	}
...
void Exporter::ApplyAllSkinOffsets(NiAVObjectRef & root) {
	...
	if (niGeom != nullptr && niGeom->IsSkin() == true) {
...
Exporter::Result Exporter::scanForIgnore(INode *node)
{
	...
	BOOL ignore = FALSE;
	if (node->GetUserPropBool(TEXT("np_ignore"), ignore))
	{
		markAsHandled(node);
	}
	for (int i = 0; i < node->NumberOfChildren(); i++) {
